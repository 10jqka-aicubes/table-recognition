...
class Table:
    ...
    def parse_table(self):
        ...
        for cell in cells:
            ...
            new_cell = Cell(
                table_id=str(self.id), start_row=sr, start_col=sc, cell_box=b_points, end_row=er, end_col=ec
            )
                                    ...
                                    for temp in range(c_from + 1, self._maxCol + 1):
                                        if tab[r][temp] != 0:
                                            if type(tab[r][temp]) == list:
                                                ...
                                                adj_relation = AdjRelation(cell, tab[r][temp], AdjRelation.DIR_HORIZ)
                                                retVal.append(adj_relation)
                                            break
                        else:
                            c_to = c_from + 1
                                    ...
                                    for temp in range(r_from + 1, self._maxRow + 1):
                                        if tab[temp][c] != 0:
                                            if type(tab[temp][c]) == list:
                                                ...
                                                adj_relation = AdjRelation(cell, tab[temp][c], AdjRelation.DIR_VERT)
                                                retVal.append(adj_relation)
                                            break
                        else:
                            r_to = r_from + 1
    ...
    def find_cell_mapping(self, target_table, iou_value):
        ...
        for cell_1 in self.table_cells:
            for cell_2 in target_table.table_cells:
                iou = cell_1.compute_cell_iou(cell_2)
                if iou >= iou_value:
